function [p, gtp, ncg1, dnew, eig_val,ind,condnum] = ...
	modlnp (d, x, g, maxit, upd1, ireset, bounds, ipivot, argvec, sfun)
%---------------------------------------------------------
% this routine performs a preconditioned conjugate-gradient
% iteration to solve the Newton equations for a search
% direction for a truncated-newton algorithm. 
% When the value of the quadratic model is sufficiently 
% reduced, the iteration is terminated.
%---------------------------------------------------------
% parameters
%
% p           - computed search direction
% g           - current gradient
% gv,gz1,v    - scratch vectors
% r           - residual
% d           - diagonal preconditoning matrix
% feval       - value of quadratic function
%------------------------------------------------------------
% initialization
%------------------------------------------------------------

global out
accrcy = argvec(1);
gnorm  = argvec(2);
xnorm  = argvec(3);

eig_val = [];
if (maxit == 0);
  p    = -g;
  gtp  = p'*g;
  ncg1 = 1;
  dnew = d;
  if (norm(p)==0);
      if out==1; disp('MODLNP 01: |p| = 0'); end
%       pause(1);
  end;
  return;
end;
first = 1;
tol   = 1.d-12;  %%%### was 1.d-12  %%%###
qold  = 0;
dnew    = initpc (d, upd1, ireset);
r     = -g;
v     = zeros(size(r));
p     = v;
gtp   = p'*g;

rho  = zeros(maxit+1,1);
beta = zeros(maxit,1);
v_gv = zeros(maxit,1);

rho(1) = norm(r)^2;

%------------------------------------------------------------
% main iteration (conjugate-gradient iterations for Ax = b)
%------------------------------------------------------------
ind = 0;
ncg1 = 0;
for k = 1:maxit
%     fprintf('\n maxit = %d \n', maxit)
   ncg1 = ncg1 + 1;
   if (bounds); r  = ztime ( r, ipivot); end;
   
   %% find the preconditioner zk here
   zk = msolve (r, upd1, ireset, first, d);
  
   if (bounds); zk = ztime (zk, ipivot); end;

   %% apply zk to our residue

   rz = r'*zk; %% inner product of <r_k, z_k>



   if (rz/gnorm < tol); 
      ind = 80;
       if (norm(p)==0); p = -g; gtp = p'*g; 
          if out==1; disp('MODLNP 02: |p| = 0'); end
       end;
       return; 
   end;

    %% compute beta coefficient (step for v [the cg search direction])

   if (k > 1) 
      beta(k) = rz/rzold;
   else
      beta(k) = 0;
   end;

   %% update the search direction for cg

   v = zk + beta(k)*v; % initialially should be rk + beta(k)*v, but here we have preconditioners 
   if (bounds); v  = ztime ( v, ipivot); end;
   %% get the hessian here


   gv = gtims (v, x, g, accrcy, xnorm, sfun);


   %%
   if (bounds); gv = ztime (gv, ipivot); end;
   v_gv(k) = v'*gv;


   if (v_gv(k)/gnorm < tol); 
       if(v_gv(k)<0)
           %disp('Non-positive Hessian Approximation'); 
           eig_val_neg = spectral_estimate (k, rho, v_gv, beta);
       end
       ind = 50; 
       if (norm(p)==0);
         if out==1; disp('MODLNP 03: |p| = 0'); end
          p=-g; gtp=p'*g;
       end;
       return; 
   end;
   % dnew = ndia3 (dnew, v, gv, r, v_gv(k));
%------------------------------------------------------------

%% compute the alpha (step for newton direction p), and residue and update it
   alpha = rz / v_gv(k);
   p = p + alpha* v;
   r = r - alpha*gv;

   rho(k+1) = norm(r).^2;
  
%%
%------------------------------------------------------------
% Compute the Lanczos spectral estimate.
%------------------------------------------------------------
   eig_val = spectral_estimate (k, rho, v_gv, beta);

%------------------------------------------------------------
% test for convergence
%------------------------------------------------------------
   gtp = p'*g;
   pr = r'*p;
   q = (gtp + pr) / 2;
   qtest = k * (1 - qold/q);
   if (qtest <= 0.5); 
       if (norm(p)==0);
          if out==1; disp('MODLNP 04: |p| = 0'); end
%           pause(1);
       end;
       return; 
   end;
   qold = q;
%------------------------------------------------------------
% perform cautionary test
%------------------------------------------------------------
   if (gtp > 0); 
       disp('non-descent direction')
       ind = 40; 
       g'*p
       p=p-alpha*v;
       g'*p
       if (norm(p)==0);
          if out==1; disp('MODLNP 05: |p| = 0'); end
%           pause(1);
       end;
       return; 
   end;
   rzold = rz;
end;
k = k-1;
%------------------------------------------------------------
% terminate algorithm
%------------------------------------------------------------
if (ind == 40)
   p   = p - alpha*v;
end;
%------------------------------------------------------------
if (ind == 50 & k <= 1)
   p = msolve (g, upd1, ireset, first, d);
   p = -p;
   if (bounds); p = ztime (p, ipivot); 
   end;
end;
%------------------------------------------------------------
if (ind == 80 & k <= 1)
   p = -g;
   if (bounds); p = ztime (p, ipivot);
   end;
end;
%-----------------------------nargin-------------------------------
% store new diagonal preconditioner
%------------------------------------------------------------

gtp  = p'*g;
ncg1 = k + 1;
if (norm(p)==0);
    if out==1; disp('MODLNP 06: |p| = 0'); end
%     pause(1);
end;




return;

%------------------------------------------------------------------------------
function eig_val = spectral_estimate (k, rho, p_Ap, beta)
global eig_max

%---------------------------------------------
% Estimate the spectrum via Lanczos.

Delta_inv = diag(1./sqrt(rho(1:k)));
B = eye(k) - diag(beta(2:k), 1);
T = Delta_inv*B'*diag(p_Ap(1:k))*B*Delta_inv;
T = 0.5*(T+T');  % This makes sure that T is symmetric!
eig_val = sort(eig(T));

eig_max=eig_val(end);

condnum = eig_val(end)/eig_val(1) ;

% fprintf('condition number = %e\n', eig_val(end)/eig_val(1));
% fprintf('estimated spectral range: [%e, %e]\n', eig_val(1), eig_val(end));
return;
